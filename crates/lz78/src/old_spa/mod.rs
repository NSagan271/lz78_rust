...
pub enum SPAParams {
    ...
    DiracDirichlet(DiracDirichletParams),
    CTW(CTWParams),
}
...
impl SPAParams {
    ...
    pub fn new_lz78(
        ...
        let default_gamma =
            default_gamma.unwrap_or(inner_spa_params.maybe_get_gamma().unwrap_or(0.5));
    ...
    pub fn new_lz78_simple(inner_spa_params: SPAParams, debug: bool) -> Self {
        let default_gamma = inner_spa_params.maybe_get_gamma().unwrap_or(0.5);
        Self::LZ78(LZ78SPAParams {
            alphabet_size: inner_spa_params.alphabet_size(),
            inner_params: Box::new(inner_spa_params),
            debug,
    ...
    pub fn try_get_dirac(&self) -> Result<&DiracDirichletParams> {
        match &self {
            SPAParams::DiracDirichlet(params) => Ok(params),
            _ => bail!("Invalid SPA parameters for DiracDirichlet SPA"),
        }
    }
    ...
    pub fn maybe_set_gamma(&mut self, gamma: f64) {
        match self {
            ...
            SPAParams::DiracDirichlet(params) => params.gamma = gamma,
        }
    }

    pub fn maybe_get_gamma(&self) -> Option<f64> {
        match self {
            SPAParams::Dirichlet(params) => Some(params.gamma),
            SPAParams::LZ78(params) => Some(params.default_gamma),
            SPAParams::DiscreteTheta(_) => None,
            SPAParams::CTW(params) => Some(params.gamma),
            SPAParams::DiracDirichlet(params) => Some(params.gamma),
        }
    }
}
...
impl ToFromBytes for SPAParams {
    fn to_bytes(&self) -> anyhow::Result<Vec<u8>> {
        ...
        match self {
            ...
            SPAParams::DiracDirichlet(params) => {
                bytes.put_u8(4);
                bytes.put_f64_le(params.gamma);
                bytes.put_f64_le(params.delta);
                bytes.extend(params.disc_params.to_bytes()?)
            }
        }
        Ok(bytes)
    }
        ...
        Self: Sized,
    {
        ...
        match tpe {
            ...
            2 => Ok(Self::DiscreteTheta(DiscreteThetaParams::from_bytes(bytes)?)),
            3 => {
                let alphabet_size = bytes.get_u32_le();
                let gamma = bytes.get_f64_le();
            ...
            4 => {
                let gamma = bytes.get_f64_le();
                let delta = bytes.get_f64_le();
                let disc_params = DiscreteThetaParams::from_bytes(bytes)?;
                Ok(Self::DiracDirichlet(DiracDirichletParams {
                    gamma,
                    delta,
                    disc_params,
                }))
