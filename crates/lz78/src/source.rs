...
impl SPA for DiscreteBinaryThetaSPA {
    fn train_on_symbol(
        ...
        self.n += 1;
        Ok(-(self.spa_for_symbol(input, params, state, None)?.log2()))
    }
    ...
    fn test_on_symbol(
        ...
        Ok(-(self
            .spa_for_symbol(input, params, state, context_syms)?
            .log2()))
    }
    ...
    S: SourceNodeSPA,
{
    ...
    pub fn generate_symbols(
        ...
        for _ in 0..n {
            ...
            // current SourceNode
            let spa = self.spa_tree.spa(state, params)?;
            ...
            self.total_log_loss += self.spa_tree.train_on_symbol(state, params, next_sym)?;
            ...
            if state.node == LZ_ROOT_IDX {
                self.spa_tree.add_new_spa(
                    prev_node,
                    next_sym,
                    S::new_with_rng(&params.inner_params, rng)?,
                );
            }
        }
...
mod tests {
    ...
    fn sanity_check_lz778_source() {
        let mut rng = thread_rng();
        let mut params = SPAParams::default_lz78_dirichlet(4);
        let mut state = params.get_new_state();
